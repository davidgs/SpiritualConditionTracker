/**
 * Enhanced production server with bundle fallback handling
 * This version includes special handling for direct bundle requests
 */
...
log('=== STARTING DEPLOYMENT SERVER WITH BUNDLE FALLBACK ===', 'DEBUG');
...
writeLog('Server started with debug logging and bundle fallback');
...
const FALLBACK_PORT = 3244; // Port for our fallback server
const expoAppDir = path.join(__dirname, 'expo-app');

log(`Configuration: PORT=${PORT}, FALLBACK_PORT=${FALLBACK_PORT}, expoAppDir=${expoAppDir}`, 'DEBUG');
writeLog(`Configuration: PORT=${PORT}, FALLBACK_PORT=${FALLBACK_PORT}, expoAppDir=${expoAppDir}`);
...
try {
  ...
  if (fs.existsSync(fixScriptPath)) {
    ...
    // Manual fixes for common module issues...
    // (same code as in the original production-server.js)
  }
} catch (err) {
...
// Create fallback server to handle bundle requests
log('Setting up fallback server for bundle requests...', 'DEBUG');
const app = express();
...
app.use('/', createProxyMiddleware({
  ...
  onError: (err, req, res) => {
    ...
    if (req.url.includes('.bundle')) {
      log(`Bundle request failed, attempting direct redirect: ${req.url}`, 'WARN');
...
// Special catch-all route for direct bundle access
app.get('*.bundle', (req, res) => {
  const bundleUrl = `http://localhost:${PORT}${req.url}`;
  log(`Direct bundle request: ${req.url} -> ${bundleUrl}`, 'DEBUG');
  res.redirect(bundleUrl);
});

// Start the fallback server
const fallbackServer = app.listen(FALLBACK_PORT, () => {
  log(`Fallback server listening on port ${FALLBACK_PORT}`, 'INFO');
  writeLog(`Fallback server listening on port ${FALLBACK_PORT}`);
});

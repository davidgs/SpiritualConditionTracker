...
async function setupTables(sqlite) {
  ...
  // Create a new table for tracking meetings with YOUR sponsor
  try {
    await sqlite.execute({
      database: DB_NAME,
      statements: `DROP TABLE IF EXISTS my_sponsor_meetings;`
    });
    console.log('[ sqliteLoader.js ] Dropped my_sponsor_meetings table for fresh setup');
  } catch (error) {
    console.error('[ sqliteLoader.js ] Error dropping my_sponsor_meetings table:', error);
  }
  
  // Create my_sponsor_meetings table
  await sqlite.execute({
    database: DB_NAME,
    statements: `
      CREATE TABLE IF NOT EXISTS my_sponsor_meetings (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        date TEXT NOT NULL,
        duration INTEGER,
        meetingType TEXT,
  ...
  console.log('[ sqliteLoader.js ] My sponsor meetings table created');
  
  // Create a table for tracking action items from sponsor meetings
  try {
    await sqlite.execute({
      database: DB_NAME,
      statements: `DROP TABLE IF EXISTS my_sponsor_tasks;`
    });
    console.log('[ sqliteLoader.js ] Dropped my_sponsor_tasks table for fresh setup');
  } catch (error) {
    console.error('[ sqliteLoader.js ] Error dropping my_sponsor_tasks table:', error);
  }
  
  // Create my_sponsor_tasks table
  await sqlite.execute({
    database: DB_NAME,
    statements: `
      CREATE TABLE IF NOT EXISTS my_sponsor_tasks (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        meetingId INTEGER,
        ...
        createdAt TEXT,
        updatedAt TEXT,
        FOREIGN KEY (meetingId) REFERENCES my_sponsor_meetings (id)
      )
    `
  });
  console.log('[ sqliteLoader.js ] My sponsor tasks table created');
}

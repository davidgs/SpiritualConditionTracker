...
export default async function initSQLiteDatabase() {
  ...
  try {
    ...
    console.log('[ sqliteLoader.js ] Falling back to in-memory storage for development...');
    ...
    return {
      async getAll(collection) {
        const items = Array.from(memoryStorage.values()).filter(item => item._collection === collection);
        return items.map(item => {
          const { _collection, ...cleanItem } = item;
          return cleanItem;
        });
      },
      ...
      async add(collection, item) {
        ...
        memoryStorage.set(`${collection}_${newId}`, { ...newItem, _collection: collection });
        console.log(`[ sqliteLoader.js ] Created ${collection} item with ID:`, newId);
        return newItem;
      },
      ...
      async update(collection, id, updates) {
        ...
        if (existing) {
          ...
          memoryStorage.set(key, updated);
          const { _collection, ...cleanItem } = updated;
          return cleanItem;
        }
        return null;
      },
      ...
      async getById(collection, id) {
        const item = memoryStorage.get(`${collection}_${id}`);
        if (item) {
          const { _collection, ...cleanItem } = item;
          return cleanItem;
        }
      ...
      async resetDatabase() {
        memoryStorage.clear();
        idCounter = 1;
        console.log('[ sqliteLoader.js ] Memory storage reset complete');
        return true;
      },

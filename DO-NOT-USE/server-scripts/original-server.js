...
const server = http.createServer((req, res) => {
  ...
  if (url === '/sqlite-test') {
    fs.readFile(path.join(__dirname, 'app/sqlite-test.html'), (err, data) => {
      if (err) {
        res.writeHead(404);
  ...
  // Serve assets
  if (url.startsWith('/app/assets/') || url.startsWith('/app/src/') || url.startsWith('/app/dist/')) {
    const assetPath = path.join(__dirname, url);
    serveStaticFile(assetPath, res);
    return;
  }
  
  // For any other path, check if it's a file in the app directory

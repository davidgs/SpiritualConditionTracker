 ...
 * @returns {boolean} - True if device screen width is less than the threshold or if user agent suggests mobile
 */
export const isMobileDevice = (threshold = 768) => {
  ...
  // Also check user agent when in web environment
  if (Platform.OS === 'web' && typeof navigator !== 'undefined') {
    ...
    // Force mobile interface when detected through user agent
    if (mobileRegex.test(userAgent) || mobileRegex2.test(userAgent.substr(0, 4))) {
      return true;
    }
    
    // Force mobile interface for touch-enabled devices
